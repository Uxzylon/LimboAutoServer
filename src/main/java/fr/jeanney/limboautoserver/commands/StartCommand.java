...
public class StartCommand implements SubCommand {
    ...
    public StartCommand(LimboAutoServer plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(CommandSource source, String[] args) {
        if (args.length != 2) {
            source.sendMessage(Component.text("Usage: /las start <serverName>"));
            return;
        }
        ...
        source.sendMessage(Component.text("Starting server \"" + serverName + "\"... Please wait."));

        plugin.getServerManager().ensureServerStarted(server).whenComplete((result, ex) -> {
            if (ex != null) {
                source.sendMessage(Component.text("Failed to start server: " + ex.getMessage())
                        .color(NamedTextColor.RED));
            } else {
                source.sendMessage(Component.text("Server \"" + serverName + "\" is online.")
    ...
    public boolean hasPermission(SimpleCommand.Invocation invocation) {
        return invocation.source().hasPermission("limboautoserver.command.start");
    }
